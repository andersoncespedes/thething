<?php
	...
	if (isset($_POST['crear'])) {
		if ($actividad->newpost($_POST) ) {
			header('location: ../../perfil/inicio.php?page_new=1');
		}
		else{
			header('location: ../../perfil/inicio.php?page_error=0');
		}
	}

	if (isset($_POST['editar'])) {
		if ($actividad->setEditActividad($_POST)) {
			header('location: ../../perfil/inicio.php?pagedit=1&id=' .$_GET['id']);
		}
		else{
			header('location: ../../perfil/inicio.php?pagedit=0&id=' .$_GET['id']);
		}
	}
	...
	if (isset($_GET['delete'])) {
		if ($actividad->deleteActividad($_GET['id'], $_GET['img'])) {
			header('location: ../../perfil/inicio.php?delete=1&id=' .$_GET['id']);
		}
		else{
			header('location: ../../perfil/inicio.php?delete=1&id=' .$_GET['id']);
		}
